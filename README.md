...
# Merge two reports (optional command)

You can merge two reports generated by `map:gaps:scan` (CSV or XLSX).  
The command uses the **BASE** report as the primary source and merges rows from the **OTHER** report on top of it.

```bash
php bin/console report:merge \
  --base data/output/missing-items.xlsx \
  --other data/output/other-scan.xlsx \
  --output data/output/missing-items-merged.xlsx
```

## What it does

- Reads both reports (CSV/XLSX) and normalizes their columns.
- Merges rows by `id`, building a new output file from scratch (no in-place edits).

- Resolves conflicts as follows:
  - If only one row (base/other) has a non-empty `name`, that row wins.
  - If both have a non-empty `name`, the BASE row wins.
  - If neither has `name`, the BASE row is kept.
- Sorting:
  - Default: `occurrences` (descending), tie-break by `id` ascending.
  - Optional: `id-asc` (ascending by `id`).
- XLSX output is auto-enhanced: filters, frozen header row, sensible alignments, autosized columns (except `image`), and minimum row height for icons.
- Images (optional): if you pass `--image-dir`, icons are re-embedded from scratch (file names `<id>.png`), avoiding conflicts with any existing images.

## Output

- Default output path (if not provided):
  - `data/output/missing-items-merged.xlsx` for XLSX
  - `data/output/missing-items-merged.csv` for CSV
- The output must be different from both input files.

Options

- `--base` (required) — path to the BASE report (`.xlsx` or `.csv`)
- `--other` (required) — path to the OTHER report (`.xlsx` or `.csv`)
- `--output` — output path; if omitted, defaults as above
- `--format` — `xlsx` | `csv` (if omitted, inferred from `--output`; defaults to xlsx)
- `--sort` — `occurrences` | `id-asc` (default: `occurrences`)
- `--image-dir` — directory with `<id>.png` files; if omitted, no images are added
- `--csv-delimiter-base` — delimiter for BASE CSV (default: `,`)
- `--csv-delimiter-other` — delimiter for OTHER CSV (default: `,`)

## Examples

### XLSX → XLSX with images, sorted by id ascending
...
--format xlsx \
--sort id-asc \
--image-dir "C:/path/to/icons"
```

### CSV → CSV (default sort = occurrences)

```bash
php bin/console report:merge \
--base data/output/missing-items.csv \
--other data/output/other-scan.csv \
--output data/output/missing-items-merged.csv \
--format csv
```

## Notes
- The merge logic only looks at the columns produced by map:gaps:scan (e.g., id, occurrences, article, name, etc.).
- For XLSX, the enhancer runs automatically after writing to apply filters, freeze the header row, alignments, and column widths; it also keeps a minimum row height so icons look crisp.
